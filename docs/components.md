...
| `Terminal`                 | ghostty-web terminal emulator with OSC 7 CWD and title tracking; calls [`restoreState()`](#useterminal-hook) on mount to replay buffered VT output from the server, restoring terminal content on page refresh or reconnection; calls [`initUrlOpener()`](#url-opener-utilities-liburl-openerts) on first mount (app-lifetime singleton shared by all Terminal instances) to patch `window.open` so ghostty-web link clicks are routed through the native Tauri opener (or standard `window.open` in browser mode)                                                                                                                                                                                                                         |
| `CodeEditor`               | Monaco Editor instance; calls [`setupMonacoLinks()`](#monaco-link-utilities-libmonaco-linksts) on mount to register a link provider and opener for clickable URLs in editor content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `TabBar`                   | Sortable tab strip — `variant` (content/bottom), context menu, inline rename, accent line, DnD via `useSortable`; accepts `onSplitRight`, `onSplitDown`, `onClosePane`, `canClosePane` for pane-splitting operations, `group` for cross-pane DnD identification; the + button supports an `onAddAgent` prop which renders an `AppDropdownMenu` dropdown with 'Terminal' and 'Agent' options; `canAddTerminal` controls whether the + button (and its dropdown menu items) are enabled and `agentStatusMap` provides per-agent status indicators                                                                                                                                                                                            |
...
| `YmirLogo`                 | Inline SVG logo component with configurable `size` prop (default 120px). Renders the Ymir "Y" shape icon as an SVG with `data-testid="ymir-logo"`. Accepts an optional `style: React.CSSProperties` prop applied to the root SVG element. Used in the branded loading screen and the empty pane state                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `SplitLeafPane`            | Leaf pane component with `TabBar`, content rendering via `PaneContent`, and split/close operations; uses `useTerminalPane` for per-pane tab management; exposes `TerminalPanelHandle` via `forwardRef` for imperative cross-pane tab transfer; implements `handleAddAgent` callback which passes `onAddAgent` to `TabBar`, enabling the + button dropdown for agent tab creation                                                                                                                                                                                                                                                                                                                                                           |
| `PaneContent`              | Shared content renderer that displays the active tab's body — delegates to `EditorPane` (editor tabs), `DiffViewer` (diff tabs), `GitTreeTab` (git-tree tabs), or a terminal container div based on `activeTab.type`. When no `activeTab` exists, renders a centered `YmirLogo` (33% width, max 150px) as the default empty state; accepts an optional `emptyState` prop to override this. The `onOpenAgent` prop has been removed. Used by both `ContentPane` (legacy) and `SplitLeafPane`                                                                                                                                                                                                                                                |
...
| `DiffViewer`               | Side-by-side / inline diff viewer for git file changes and commit diffs; calls [`setupMonacoLinks()`](#monaco-link-utilities-libmonaco-linksts) on mount to register a link provider and opener for clickable URLs in diff content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
## URL Opener Utilities (`lib/url-opener.ts`)

Shared URL-opening infrastructure used by both the terminal and Monaco editor. Provides scheme detection, external-url opening, and a global `window.open` override for Tauri mode.

| Export                       | Purpose                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `URL_SCHEME_PREFIXES`        | `readonly` array of 11 supported URL scheme prefixes: `https://`, `http://`, `mailto:`, `ftp://`, `ssh://`, `git://`, `tel:`, `magnet:`, `gemini://`, `gopher://`, `news:`. Single source of truth for the regex and prefix check.                                                                                                           |
| `URL_SCHEME_REGEX`           | `RegExp` derived from `URL_SCHEME_PREFIXES` (each prefix is regex-escaped and joined with `                                                                                                                                                                                                                                                  | `). Used by [`setupMonacoLinks`](#monaco-link-utilities-libmonaco-linksts) to scan editor content for URLs. Global, case-insensitive (`'gi'`). |
| `stripTrailingPunctuation()` | `(url: string) => string` — strips trailing characters in `/[.,;!?\]]+$/` from a URL. Handles cases where surrounding text punctuation is captured by the regex match.                                                                                                                                                                       |
| `openExternalUrl()`          | `(url: string) => Promise<void>` — opens a URL in the system browser. In Tauri mode, uses `@tauri-apps/plugin-opener` (falls back to saved `window.open` on plugin failure). In browser mode, calls `window.open` directly.                                                                                                                  |
| `initUrlOpener()`            | `() => () => void` — patches `window.open` globally in Tauri mode so that ghostty-web terminal link clicks are routed through `openExternalUrl` instead of opening a new webview window. Returns a cleanup function (intended to restore the original `window.open`, but never invoked — see singleton behavior below). No-op outside Tauri. |

**`initUrlOpener()` internals:**

- Saves the original `window.open` before patching (also stored in a module-level `_originalWindowOpen` so `openExternalUrl`'s fallback path avoids infinite recursion)
- The patched `window.open` checks URLs synchronously against `URL_SCHEME_PREFIXES` — external URLs are routed to `openExternalUrl` (fire-and-forget), all other URLs pass through to the original `window.open`
- **Used by:** [`Terminal`](#key-components) component — called once on first Terminal mount; the `window.open` override persists for the entire app session and is shared by all Terminal instances. The cleanup function is not invoked on Terminal unmount.

## Monaco Link Utilities (`lib/monaco-links.ts`)

Registers Monaco Editor link detection and opening for external URLs. Eliminates duplication between [`CodeEditor`](#key-components) and [`DiffViewer`](#key-components), both of which call `setupMonacoLinks()` on mount.

### `setupMonacoLinks(monaco)`

```tsx
const disposable = setupMonacoLinks(monaco);
// ... later
disposable.dispose();
```

Accepts the Monaco module and returns a combined `{ dispose() }` object that cleans up both the link provider and link opener registrations. Each call creates independent registrations — multiple callers may safely call this function concurrently.

**Link Provider** — registered globally (`'*'` language). Scans every line of a model using `URL_SCHEME_REGEX` (fresh `RegExp` per call to reset `lastIndex`). Strips trailing punctuation from each match via `stripTrailingPunctuation()`. Produces Monaco link objects with `range`, `url`, and `tooltip: 'Ctrl+Click to open'`.

**Link Opener** — registered globally. Delegates to `openExternalUrl()` for all link activations.

**Used by:** `CodeEditor` and `DiffViewer` — each calls `setupMonacoLinks(monaco)` on mount and disposes on unmount.
