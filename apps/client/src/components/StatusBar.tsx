...
export function StatusBar({ activeWorkspaceName }: StatusBarProps) {
  ...
  const statusText = isConnected ? 'Connected' : isReconnecting ? 'Reconnecting...' : 'Disconnected';

  return (
    <div data-testid="status-bar" style={{
      height: '22px', background: '#007acc', color: '#fff',
      display: 'flex', alignItems: 'center', padding: '0 8px',
      fontSize: '12px', gap: '16px', flexShrink: 0,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        <div data-testid="status-indicator" style={{ width: '8px', height: '8px', borderRadius: '50%', background: statusColor }} />
