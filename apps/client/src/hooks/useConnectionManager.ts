...
export function useConnectionManager(): UseConnectionManagerReturn {
  ...
  const [localPort, setLocalPort] = useState<number | null>(() => getSidecarPort());

  // Re-sync localPort if __YMIR_SIDECAR_PORT changes (e.g. set by useAuth
  // after Tauri config is loaded).
  useEffect(() => {
