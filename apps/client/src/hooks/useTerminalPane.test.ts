...
describe('useTerminalPane', () => {
  ...
  // -----------------------------------------------------------------------
  // 7. Race condition — rapid workspace switching ignores stale responses
  // -----------------------------------------------------------------------
  test('ignores stale tab.list responses from previous workspace after rapid switch', async () => {
    ...
    // Wait for the first tab.list request (for ws-1)
    await waitFor(() => {
      expect(getAllSentEnvelopes().some((e) => e.channel === 'tab.list')).toBe(true);
    });

    // Capture the first request's ID before switching
    const firstRequest = getAllSentEnvelopes().find((e) => e.channel === 'tab.list')!;
    const firstRequestId = firstRequest.id;

    // Clear sent envelopes so we can isolate ws-2's request
    mockSend.mockClear();

    // Switch to ws-2 WITHOUT responding to ws-1's tab.list first
    rerender({ workspaceId: 'ws-2' });

    // Wait for the second tab.list request (for ws-2)
    await waitFor(() => {
      expect(getAllSentEnvelopes().some((e) => e.channel === 'tab.list')).toBe(true);
    });

    const secondRequest = getAllSentEnvelopes().find((e) => e.channel === 'tab.list')!;
    const secondRequestId = secondRequest.id;

    // Respond to ws-2's request first
    const ws2Tabs = [
      {
        id: 'tab-ws2-1',
        tabType: 'terminal' as const,
    ...
    await act(async () => {
      simulateResponse(secondRequestId!, { tabs: ws2Tabs });
    });

    // Verify ws-2 tabs are loaded
    await waitFor(() => {
      expect(result.current.tabs).toHaveLength(1);
      expect(result.current.tabs[0].title).toBe('WS2 Terminal');
    });

    // Now respond to ws-1's tab.list (stale response — should be ignored)
    const ws1Tabs = [
      {
        id: 'tab-ws1-1',
        tabType: 'terminal' as const,
    ...
    await act(async () => {
      simulateResponse(firstRequestId!, { tabs: ws1Tabs });
    });

    // The stale response must NOT overwrite ws-2's tabs
    expect(result.current.tabs).toHaveLength(1);
    expect(result.current.tabs[0].title).toBe('WS2 Terminal');
    expect(result.current.tabs[0].id).toBe('tab-ws2-1');

    // Only ws-2's terminal should have been registered
    expect(onTerminalRegistered).toHaveBeenCalledTimes(1);
    expect(onTerminalRegistered).toHaveBeenCalledWith('term-ws2-1', 'tab-ws2-1', 'ws-2');
  });
});
